...
from classical3dB_fun import classical3dB_inverse_poly

# 正弦波叠加法之经典谱的实现
# 里面包括了六种仿真平坦衰落信道的方法- 等距离法、等面积法、 蒙特卡洛法、最小均方误差法、精确多普勒扩展法和Jakers仿真法

def parameter_classical3dB(Method_type,N_i,Variance,fd,phase) :
#初始化
...
# 生成固定的系数
     if Method_type == 'MEA' :
          ...
          for i in range(N_i):
               f_i = np.append(f_i,classical3dB_inverse_poly(var/2*(1+n[i]/N_i)))
